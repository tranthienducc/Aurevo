import { FrameShape, Shape } from "@/redux/slices/shaped";

export const isShapeInFrame = (shape: Shape, frame: FrameShape): boolean => {
  const frameLeft = frame.x;
  const frameTop = frame.y;
  const frameRight = frame.x + frame.w;
  const frameBottom = frame.w + frame.h;
...
const getShapesInSideFrame = (shapes: Shape[], frame: FrameShape): Shape[] => {
  const shapesInFrame = shapes.filter(
    (shape) => shape.id !== frame.id && isShapeInFrame(shape, frame)
  );
...
export const dowloadBlob = (blob: Blob, filename: string): void => {
